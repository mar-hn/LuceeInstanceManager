...
app.on('ready',() =>
{
  ...
  //$FIX Not reading hombrew installed commands
  //exec('PATH="${PATH}:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" && export $PATH',{encoding:"utf8"});
...
autoUpdater.on('error', (event, error) => 
{
  ...
  dialog.showErrorBox('Lucee Instance Manager general protection fault', event + ' :: ' + error);  
})
...
autoUpdater.on('update-available', () => {
  ...
  }, (buttonIndex) => 
  {
    if (buttonIndex === 0) 
    {
      ...
      exec(`sudo -u ${LoggedUser} open https://github.com/mar-hn/LuceeInstanceManager/releases/`);
    }
